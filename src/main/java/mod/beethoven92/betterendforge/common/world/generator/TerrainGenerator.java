...
public class TerrainGenerator {
	...
	private static float getAverageDepth(BiomeProvider biomeSource, int x, int z) {
		if (getBiome(biomeSource, x, z).getBaseHeight() < 0.1F) {
			return 0F;
		}
		float depth = 0F;
		for (int i = 0; i < OFFS.length; i++) {
			int px = x + OFFS[i].x;
			int pz = z + OFFS[i].y;
			depth += getBiome(biomeSource, px, pz).getBaseHeight() * COEF[i];
		}
		return depth;
	}

	private static Biome getBiome(BiomeProvider biomeSource, int x, int z) {
		if(biomeSource instanceof BetterEndBiomeProvider) {
			return ((BetterEndBiomeProvider)biomeSource).getNoiseBiome(x, 0, z);
		}
		return Biomes.SKY;
	}
