...
public class SplineHelper 
{
	public static List<Vector3f> makeSpline(float x1, float y1, float z1, float x2, float y2, float z2, int points) 
	{
		...
		for (int i = 1; i < count; i++) {
			...
			float x = AdvMathHelper.lerp(delta, x1, x2);
			float y = AdvMathHelper.lerp(delta, y1, y2);
			float z = AdvMathHelper.lerp(delta, z1, z2);
	...
	public static SDF buildSDF(List<Vector3f> spline, float radius1, float radius2, Function<BlockPos, IBlockState> placerFunction)
	{
		int count = spline.size();
		float max = count - 2;
		SDF result = null;
		...
		for (int i = 1; i < count; i++) 
		{
			...
			SDF line = new SDFLine()
					.setRadius(AdvMathHelper.lerp(delta, radius1, radius2))
	...
	public static boolean fillSpline(List<Vector3f> spline, World world, IBlockState state, BlockPos pos, Function<IBlockState, Boolean> replace)
	{
	...
	public static void fillSplineForce(List<Vector3f> spline, World world, IBlockState state, BlockPos pos, Function<IBlockState, Boolean> replace) {
		Vector3f startPos = spline.get(0);
		for (int i = 1; i < spline.size(); i++) {
			Vector3f endPos = spline.get(i);
	...
	public static void fillLineForce(Vector3f start, Vector3f end, World world, IBlockState state, BlockPos pos, Function<IBlockState, Boolean> replace) {
	...
	private static boolean fillLine(Vector3f start, Vector3f end, World world, IBlockState state, BlockPos pos, Function<IBlockState, Boolean> replace)
	{
		float dx = end.getX() - start.getX();
		float dy = end.getY() - start.getY();
		float dz = end.getZ() - start.getZ();
	...
	public static boolean canGenerate(List<Vector3f> spline, float scale, BlockPos start, World world, Function<IBlockState, Boolean> canReplace)
	{
		int count = spline.size();
		...
		for (int i = 1; i < count; i++) {
			...
			for (float py = y1; py < y2; py += 3) {
				...
				float x = AdvMathHelper.lerp(lerp, x1, x2);
				float z = AdvMathHelper.lerp(lerp, z1, z2);
				mut.setPos(x, py, z);
	...
	public static boolean canGenerate(List<Vector3f> spline, BlockPos start, World world, Function<IBlockState, Boolean> canReplace)
	{
		int count = spline.size();
		Vector3f vec = spline.get(0);
		BlockPos.MutableBlockPos mut = new BlockPos.MutableBlockPos();
	...
	public static Vector3f getPos(List<Vector3f> spline, float index) 
	{
		...
		float x = AdvMathHelper.lerp(delta, p1.getX(), p2.getX());
		float y = AdvMathHelper.lerp(delta, p1.getY(), p2.getY());
		float z = AdvMathHelper.lerp(delta, p1.getZ(), p2.getZ());
